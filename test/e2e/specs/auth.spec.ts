...
test.describe('Auth — edge cases', () => {
  ...
  test('should show error when signing up with already-registered email', async ({
    ...
    // path actually reaches the backend (display name + password).
...
test.describe('Auth — sign-up happy path', () => {
  ...
  test('should create a new account and land on the onboarding screen', async ({
    ...
    // Option A — the full signup form requires a display name + a password
    // before the validators pass. (Confirm field dropped — the reveal toggle
    // is the typo-safety net.)
  ...
  test('should NOT submit when password field keyboard-Done is pressed with age gate unticked', async ({
    page,
  }) => {
    // The password field's onFieldSubmitted is wired to null when the age
    // checkbox is unticked (see login_screen.dart: `onFieldSubmitted: isLoading
    // || (_isSignUp && !_ageConfirmed) ? null : (_) => _submit()`). With the
    // confirm field dropped (UX option a), the password field is now the
    // keyboard-submit point. Pressing Enter/Done while the checkbox is unticked
    // must be a structural no-op — no submission, no navigation.
    //
    ...
    //
    // Reliability note: after flutterFill the password field retains focus in
    ...
    await flutterFill(page, AUTH.emailInput, throwawayEmail);
    // flutterFill on the password field leaves it focused — it is now the
    // keyboard-submit point.
    await flutterFill(page, AUTH.passwordInput, 'TestPass123!');
...
test.describe('Auth — signup age gate', { tag: '@smoke' }, () => {
  ...
  test('should show the full-form signup surfaces in signup mode (Option A)', async ({
    ...
    // Option A added a display-name field, a password-strength bar, and a
    // "CREATE ACCOUNT" heading — all signup-only. (The confirm-password field
    // was dropped — the reveal toggle replaces its typo-safety role.)
    await expect(page.locator(AUTH.signupHeading).first()).toBeVisible({
      timeout: 5_000,
    });
