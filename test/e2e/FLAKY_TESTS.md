...
**State as of 2026-06-17 (Phase 38e QA gate, PR #344):** `saga.spec.ts` S8 ("should compose the 3 deep-dive sub-widgets") failed on CI shard 3 (failed on initial run + retry #1) with `scrollIntoViewIfNeeded: Timeout 15000ms`. NOT a production regression — the cardio activation flip legitimately grew the /saga/stats deep-dive (a 7th vitality row + a 7th trend line), pushing the lazily-built VOLUME & PEAK blocks further past the initial ListView build window (cluster `listview-lazy-build-breaks-e2e`). The existing `wheelScrollUntilMounted` helper polled mount-count then called `scrollIntoViewIfNeeded` on the recycled virtualized child, which is unreliable against a Flutter list node. **Fix (deterministic, NOT tagged `@flaky`):** replaced with `wheelScrollUntilVisible` — wheel-scrolls until the element is actually VISIBLE (mounts AND positions it), raised step budget 16→30 for a short CI viewport, and dropped the fragile `scrollIntoViewIfNeeded` follow-up. Verified 8/8 in the describe + 3/3 on S8 alone at workers=3. The transient "Wrong email or password" seen during a local 2-file run is the known local-Supabase `sign_in_sign_ups` rate-limit (carryover #14), not a new flake.
