...
| 22 | `specs/workouts.spec.ts` | `Workout loading overlay cancel (PR1 — Q1)` → `should show Cancel button immediately on loading overlay and restore workout on tap (PR1 — Q1)` | **RESOLVED in PR #198 (PR-2 of audit fix wave).** Under `--repeat-each=5` (4 workers): used to fail 4/5 at the cleanup discard nav assertion (line 1449: `toBeVisible(nav-home, 15s)`). Also surfaced as a single-run failure on PR-2's initial CI (the cleanup chain timed out under GHA load even without repeat-each). The test's PRIMARY assertions (Cancel visible from t=0, workout restored, overlay dismissed) always passed — the flake was exclusively on the final cleanup discard path. Fix: replaced cleanup discard chain with direct `page.goto('/')` per QA's recommended fix class. Discard-as-cleanup was incidental; the Q1 product contract is unaffected. `@flaky` tag retained pending one full green CI cycle, then will be removed and this entry deleted on PR #198 merge. |
