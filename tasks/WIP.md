...
- [x] reviewer: pass; address every finding before merge
  ...
  - [x] tech-lead: Suggestion (extract `SyncErrorClassifier.httpCode()` helper) — explicitly deferred per orchestrator instruction; not in scope for the revision pass.
  - [x] tech-lead: `dart format` clean, `dart analyze --fatal-infos` clean, full unit/widget suite green: 2394 → 2395 (+1 new classification test; widget-side delta is 0 because the existing `tapping cancel...` test was strengthened in-place rather than duplicated).
