...
- **Deferred follow-ups (post-Phase-20 polish train, all DONE except where noted):** Rename pt-BR `Rotinas → Treinos` + `Sessão` (PR #158) · Match indicator "= last set" — Pillar 1 (PR #159) · Set-type long-press discoverability — persistent `WK/WU/DR/FL` micro-label (PR #160) · Validation audit (PR #161) · Bodyweight row weight-column hide + pending-FL label color (PR #163). **Still deferred:** post-completion hint persistence (first attempt re-triggered the Flutter Web role-swap bug; needs layout-stable redesign) and a manual on-device walkthrough — both in the **Active Backlog** section below.
...
- **Routines-rename + S12 saga E2E flakes** — RESOLVED 2026-05-11 on branch `fix/e2e-flakes-routines-rename-and-s12-saga`. Both root-caused: routines was a `flutterLongPress` pointer-event jitter (test-infra), S12 was a too-tight 60 s test timeout for the longest user flow (test-infra). Fixes verified 40/40 (routines) and 20/20 (S12) consecutive passes locally. CI workflow improvement (`--grep-invert "@flaky"` mirroring QA's local pattern) remains an open infrastructure question but is no longer blocking — `FLAKY_TESTS.md` "Active flaky tests" table is now empty.
...
- `routines.spec.ts` "should edit a routine name" + "should delete a routine and remove it from the list" — RESOLVED 2026-05-11. Root cause was `flutterLongPress` helper occasionally firing `onTap` instead of `onLongPress` (Chromium pointer-event jitter rejecting Flutter's `LongPressGestureRecognizer` mid-hold). Fix landed in `helpers/app.ts`: re-anchor cursor with `mouse.move(cx, cy)` after `mouse.down()` and before `mouse.up()`, plus default hold raised 800 ms → 1000 ms. Verified 40/40 consecutive passes locally (15 + 15 + 30 across edit/delete combinations under `--repeat-each`).
- `saga.spec.ts:437` S12 "should update class badge after chest crosses rank 5" — RESOLVED 2026-05-11. Root cause was the 60 s test timeout being too tight for the longest single-user flow in the suite (full workout + multi-overlay celebration sequence + character-sheet reload). Fix: `test.setTimeout(120_000)` on the test, `@flaky` tag removed. Verified 20/20 consecutive passes locally (10 + 10 with `--repeat-each`). Same branch as routines fix.
