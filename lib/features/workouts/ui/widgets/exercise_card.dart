...
// Phase 38b: the card header (was `_ExerciseCardHeader`) moved to
// `exercise_card_header.dart` as the public [ExerciseCardHeader] so the
// cardio `CardioEntryCard` shares the exact same shell chrome.
...
///
/// Phase 38b: promoted from private (`_ExerciseDetailSheet`) so the cardio
/// `CardioEntryCard`'s info affordance opens the SAME sheet as the strength
/// card — one detail surface, both modalities.
class ExerciseDetailSheet extends ConsumerWidget {
  const ExerciseDetailSheet({required this.exercise, super.key});

  final Exercise exercise;

  /// Canonical entry point — presents the sheet with the modal chrome both
  /// card types use.
