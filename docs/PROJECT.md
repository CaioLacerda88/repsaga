...
| Vitality-2 | Per-body-part conditioning delta — replaces the post-session "Conditioning charged +N%" aggregate (diluted by maxed parts + crushed by the frozen all-time `vitality_peak` → invisible for detrained users) with a per-bp **rune charge strip** (three-way gainer / MÁX / HELD, never a dead `+0`, delta-ordered, 4 rows + "+N more", all-at-peak banner, guard line) + a charge rune **fused onto the B2 cinematic hero beat** (single charge source feeds both → no drift; cascade hero-row only; additive when no charge data). New decaying **`vitality_ref_peak`** denominator (migration **00083**: 21-day half-life `GREATEST(new_ewma, prior_ref_peak*decay)` maintained in `recompute_vitality_for_user`, mirrored in `vitality-nightly` edge fn for save/nightly parity; backfill `ref_peak = vitality_peak` zero-discontinuity) — `vitality_peak`/Saga "career best" untouched; charge = `clamp(ewma/ref_peak)`. Guard surfaced client-side (`before.ewma == after.ewma` → "já carregado hoje", no RPC signature change). Safety (Phase-39/ToS): fill-only runes never drain, past-tense descriptive copy, no decay countdown. First pgTAP coverage of `recompute_vitality_for_user` (10 assertions). Diagnosed on-device (instrumented controller + controlled `last_vitality_date` reset proved client wiring correct — design defect, not a bug). Reviewer (1 Blocker: held-not-max rendered `+0` → three-way classification) + QA + visual gate (320/360/412dp) signed off; CI green; **00083 pushed to hosted + backfill verified**. New memory `feedback_local_ci_gates_beyond_analyze` (run shell gates, not just `dart analyze`, when `make` is unavailable). | DONE | #406 |
| Vitality-3 | Strength XP vitality gate — gates **strength** set XP by per-bp conditioning: `vmult = 0.50 + 0.50×clamp(vitality_ewma/vitality_ref_peak)`, per body part, computed pre-session, applied as the 12th/final factor. **Throttle-only** (ranks stay monotonic — vitality only slows earn-rate; rank never decays). Denominator = the decaying `vitality_ref_peak` (not frozen `vitality_peak`), so a consistent lifter sits at full charge (`vmult 1.0`, no-op) while a sandbagger/detrained part is throttled toward the 0.50 floor and self-corrects as ref_peak re-baselines. **Science-validated** (product-owner + WebSearch): muscle-memory honored — returner's vmult recovers 0.50→0.92→1.0 over wk1-3 (Halonen 2024, Cumming 2024) and `tier_diff_mult` already over-rewards returners so the dip is never punitive; load-fed EWMA severs the feedback loop. **Recalibration: none needed** — measured `VPCT_NORMAL=1.0` → `STRENGTH_BASE_RECENTER=1.0`, base economy untouched for consistent lifters (existing fixture byte-identical). Built the per-bp vitality model INTO the 13-persona calibration sim (modeled zero before; daily-grid EWMA mirroring 00083) + 2 new personas (sandbagger ranks below advanced; detrained-returner rank holds in layoff); **15/15**. Atomic 4-site adoption: sim → fixture (additive) → Dart `xp_calculator` → migration **00084** (`rpg_strength_vitality_mult` helper + `record_set_xp`/`record_session_xp_batch`, per-bp pre-session gate, verbatim-diff vs 00065/00081, signatures unchanged, no schema change). Per-bp keying proven (lapsed dominant bp doesn't bleed onto charged minors); `_rpg_backfill_chunk` left ungated (no retroactive XP). **Opportunistic fix:** corrected the mis-specified `Smurf ≤ TrueBeginner` anti-cheese invariant → `smurf+fake ≤ smurf-honest` (proved faking a 1RM is a net XP loss, robust even at 220×1). Reviewer (no findings) + QA (E2E impact clean; integration 0 new failures vs main) signed off; CI green; **00084 pushed to hosted + helper smoke-verified live** (chest@2% → vmult 0.509). Backend/formula-only (mechanic-only, D5) — no UI/visual gate. | DONE | #408 |
...
| Vitality-4 | **Save-time immediacy via day-base re-step** — fixes the root cause that made vitality-immediacy/Vitality-2 effectively dead. The `vitality-nightly` cron (03:00 UTC daily, mig 00042) stamps `last_vitality_date=today` on ALL body parts BEFORE the user trains, so `save_workout`'s in-txn `recompute_vitality_for_user` was always guard-blocked (`WHERE last_vitality_date IS DISTINCT FROM v_today` skips already-stamped parts) → finished sessions stepped ZERO vitality → conditioning charge read "mantido" on a just-trained part (dead for anyone training after 03:00 UTC; ALWAYS for Brazil UTC−3 = cron at local midnight). Diagnosed from live hosted data (back/legs `updated_at=03:00` cron-only vs trained parts' XP-reversal `updated_at`; all parts `last_vitality_date=today`). Migration **00085**: added nullable `vitality_{ewma,peak,ref_peak}_day_base` columns; the RPC no longer SKIPS a part stepped today — it RE-STEPS from the stored start-of-day base with the now-complete 7d window (first step snapshots base=live prior; re-step reads stored base), so α applies once/day from the right base (no double-count, immediacy restored). Cron stays at 03:00 (timing now irrelevant — save always re-steps from the base the cron snapshotted). **Zero formula change** at the 4 parity sites (Python sim already ticks daily-from-base → EWMA math byte-identical; only base-selection changed); pre-migration NULL day_base → `COALESCE` to live value. Tests: pgTAP block (c) reframed (re-step idempotent on unchanged window) + new block (e) (steps UP on grown window); integration reframed guard-skip→re-step + new cron-then-save STEP-UP regression; **pgTAP 13/13 + integration 15/15**. Reviewer (hand-traced idempotency / peak-monotonicity / NULL-fallback, no findings) + QA (backend-only, E2E impact none) signed off; CI green; **00085 pushed to hosted + day-base cols verified live**. User-locked Option A (day-base) over the lighter "reschedule cron to end-of-day" (C) for timezone-independence + twice-a-day correctness. | DONE | #412 |
| last-lifted-seed-ordering | **Routine seeds 0kg instead of last-lifted weight** (on-device bug). `getLastWorkoutSets` ordered with `.order('finished_at', referencedTable:'workouts')` then deduped "first per exercise" — but PostgREST orders the EMBEDDED to-one `workouts` resource, NOT the parent `workout_exercises` rows, so rows came back PK/insertion-ordered and the dedup picked an arbitrary OLD session (verified live: a months-old 0kg row won over the recent 60kg). Fix: sort rows by `workouts.finished_at` DESC **client-side before the dedup** (`_embeddedFinishedAt` reads finished_at whether the embed is a Map or single-element List; null sorts last); no query/signature/return-shape change. 3 behavior-pinned unit tests (most-recent wins despite out-of-order rows; per-exercise split-agnostic dedup; null finished_at last) — the fake returns `.order()` data verbatim, so deleting the sort turns them red. Reviewer (clean) + QA (E2E impact none — existing prefill specs single-session/target-driven; **91/91 unit**) signed off; CI green. No migration. | DONE | #411 |
