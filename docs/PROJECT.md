...
| 32g | Workout-flow hotfix wave + critical E2E coverage — duration UTC-offset fix, `dart:developer` → `debugPrint` sweep across notifier + local storage + celebration orchestrator, title equip error handler, confirm-banner persistence, 5 critical E2E specs (empty-session guard / post-session leave-confirm / class-change cinematic / server-error vs offline-error copy / tap-chip → routine sheet), 3 widget tests (Mission Debrief top-N / rest-timer countdown / 6-BP debrief layout), decimal+bodyweight XP integration, day-0 ActionHero fixture, CI grep gate against `dart:developer.log` reintroduction in workouts/RPG layers | 5-7d | NOT STARTED |
