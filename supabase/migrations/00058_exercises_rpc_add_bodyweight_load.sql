-- =============================================================================
-- Phase 24c bug fix — exercise RPCs (00034 + 00044) didn't include
-- `uses_bodyweight_load` in their RETURNS TABLE projection. The column was
-- added to `exercises` in 00056_add_bodyweight_load_semantics, but because
...
-- `Exercise` model auto-deserialized `usesBodyweightLoad` as the
-- `@Default(false)` value for every RPC-fetched row, defeating the
-- bodyweight-prompt coordinator's gate check (Phase 24c-8).
--
-- Symptom that surfaced this bug:
--   `test/e2e/specs/bodyweight-prompt.spec.ts` — the smoke gate for
--   bodyweight-prompt visibility — failed because the active workout's
--   exercise picker fed `usesBodyweightLoad: false` to the coordinator
--   despite Pull-Up being correctly flagged in the database (00056). No
--   prompt fired, the test waited 10 s for the SnackBar, then timed out.
--
-- Fix scope:
--   For each of the four exercise RPCs we re-issue `CREATE OR REPLACE
--   FUNCTION` with `uses_bodyweight_load BOOLEAN` appended to the RETURNS
--   TABLE shape and `e.uses_bodyweight_load` appended to the SELECT
--   projection. Every other line — signature, security, validation,
--   LATERAL cascade, ORDER BY — is preserved verbatim from the latest
--   in-tree definition (00034 for #1, #2, #4; 00044 for #3, which carries
--   the BUG-003 8-arg `p_id` signature on top of 00034).
--
-- Append-only — DO NOT edit 00034 or 00044 in place. Migrations are
-- immutable once shipped; the only correct way to evolve a function is to
-- replace it forward.
--
...
-- Adding `uses_bodyweight_load` to the projection IS such a shape change,
...
-- =============================================================================
-- 1. fn_exercises_localized — list/lookup RPC (00034 §6.1)
-- =============================================================================
...
RETURNS TABLE (
  ...
  slug                 TEXT,
  uses_bodyweight_load BOOLEAN  -- Phase 24c bug fix
)
LANGUAGE plpgsql
SECURITY INVOKER
STABLE
...
BEGIN
  ...
  SELECT
    ...
    e.created_at,
    e.slug,
    e.uses_bodyweight_load  -- Phase 24c bug fix
  FROM exercises e
  LEFT JOIN LATERAL (
    SELECT
...
-- =============================================================================
-- 2. fn_search_exercises_localized — trigram search RPC (00034 §6.2)
-- =============================================================================
...
BEGIN
  ...
  WITH matches AS (
    ...
    SELECT DISTINCT ON (e.id)
      ...
      e.created_at,
      e.slug,
      e.uses_bodyweight_load,  -- Phase 24c bug fix — propagate through CTE
  ...
  SELECT
    ...
    m.created_at,
    m.slug,
    m.uses_bodyweight_load  -- Phase 24c bug fix
  FROM matches m
  LEFT JOIN LATERAL (
    SELECT
      COALESCE(t_locale.name,        t_en.name,        t_any.name)        AS name,
...
-- =============================================================================
-- 3. fn_insert_user_exercise — create user-owned exercise (00044 §BUG-003,
--    8-arg variant: signature MUST match 00044 to keep the prior overload
...
BEGIN
  ...
  -- exercise (curation is the database team's call, not the user's). The
  -- column appears in the RETURNS TABLE so the post-insert SELECT below
  -- carries the false value back to the client.
  ...
  -- uses_bodyweight_load (Phase 24c bug fix above), so this composition
  -- transparently surfaces the new column.
  RETURN QUERY
...
-- =============================================================================
-- 4. fn_update_user_exercise — edit user-owned exercise (00034 §6.4)
-- =============================================================================
...
BEGIN
  ...
  -- uses_bodyweight_load is intentionally NOT touched here — same rationale
  -- as fn_insert_user_exercise above (it is a curation flag, not a
  -- user-editable field).
  UPDATE exercises e
  SET
  ...
  -- fn_exercises_localized which now projects uses_bodyweight_load.
  RETURN QUERY
  SELECT * FROM public.fn_exercises_localized(
    v_locale,
    v_owner_id,
...
-- =============================================================================
-- Sanity asserts — every replaced function must have uses_bodyweight_load
-- in its return type. A future drive-by edit that accidentally rolls back
-- the projection should fail at migration time, not at first failing E2E.
-- =============================================================================
DO $$
DECLARE
  v_fn TEXT;
  ...
  FOREACH v_fn IN ARRAY v_fns LOOP
    ...
    SELECT EXISTS (
      ...
      WHERE n.nspname = 'public'
        AND p.proname = v_fn
        AND args.mode = 't'
        AND args.name = 'uses_bodyweight_load'
    )
    INTO v_has_col;

    IF NOT v_has_col THEN
      RAISE EXCEPTION
        'Phase 24c bug-fix invariant violated: function public.% does not project uses_bodyweight_load. Re-check the RETURNS TABLE shape.',
        v_fn;
    END IF;
  END LOOP;
END
...
-- bodyweight-prompt path would stay broken until the next idle cache
-- refresh (up to 10 minutes).
