-- =============================================================================
-- 00080 — Phase 38e: cardio counts toward Character Level (atomic boundary flip)
--
-- 38a–38d compute + store cardio XP in `body_part_progress['cardio']` and
-- `xp_events` (attribution `{"cardio": <xp>}`), but cardio was excluded from
-- the Character Level reduction — the `character_state` view and every in-RPC
-- char-level snapshot filtered `body_part IN (6 strength tracks)`. 38e flips
-- that: cardio joins the active set so it contributes to Character Level,
-- matching the Dart side (`activeBodyParts`, both `_activeKeys`).
--
-- ## What changes
--   * `character_state` view: active filter 6 → 7 parts (+cardio).
--   * `record_set_xp` + `record_session_xp_batch` in-RPC char-level snapshots
--     (pre + post, used for the §8.2 character-level TITLE crossings): 6 → 7.
--   * `_rpg_backfill_chunk`: NO change — it never computes Character Level
--     (it only accumulates per-set XP; title crossings are evaluated by the
--     two writers above). Confirmed by grep: no `body_part IN (...)` filter
--     in that function.
--
-- ## Invariants preserved
--   * **Denominator stays 4.** Only the active-part SET grows. A pure-strength
--     user with cardio at rank 1 gains `+1` to both Σ ranks and N_active, so
--     the numerator `Σ ranks − N_active` is unchanged — their level NEVER
--     regresses (the thesis guard; pinned by a named Dart test).
--   * **Title thresholds UNCHANGED.** `saga_eternal` stays at level 148; the
--     172-cap title is Phase 38f. Only the COMPUTED max level rises (148→172).
--     The §8.2 VALUES lists below are byte-identical to 00077.
--
-- ## Why the full function bodies are reproduced
--   Postgres `CREATE OR REPLACE FUNCTION` requires the entire body; there is
--   no in-place line patch. These are verbatim copies of the 00077 bodies
--   with ONLY the four `body_part IN (6)` char-level filters widened to 7.
--   The single canonical 7-part list now lives in
--   `rpg_active_body_part_level(p_user_id)` so a future flip touches ONE
--   function — the two writers and the view all call it.
-- =============================================================================

-- ---------------------------------------------------------------------------
-- Canonical Character-Level reduction (single source of the 7-part active set)
-- ---------------------------------------------------------------------------
-- floor((Σ active_ranks − N_active) / 4) + 1, floored at 1, over the seven
-- active tracks (six strength + cardio). Mirrors Dart `characterLevel(...)`
-- and the `character_state` view. SECURITY DEFINER so the SECURITY DEFINER
-- writers can call it without RLS surprises; it only reads the caller-scoped
-- user's own rows (callers always pass their own v_user_id).
CREATE OR REPLACE FUNCTION public.rpg_active_body_part_level(p_user_id uuid)
RETURNS int
LANGUAGE sql
STABLE
SECURITY DEFINER
...
REVOKE EXECUTE ON FUNCTION public.rpg_active_body_part_level(uuid) FROM PUBLIC, anon;
GRANT  EXECUTE ON FUNCTION public.rpg_active_body_part_level(uuid) TO authenticated;

-- ---------------------------------------------------------------------------
-- character_state view — active filter 6 → 7 (verbatim 00040 §9 + cardio)
-- ---------------------------------------------------------------------------
...
FROM public.body_part_progress
WHERE body_part IN ('chest','back','legs','shoulders','arms','core','cardio')
GROUP BY user_id;

-- ---------------------------------------------------------------------------
-- PART A — record_set_xp (verbatim 00077 PART A + 7-part char-level snapshots)
-- ---------------------------------------------------------------------------
...
BEGIN
  ...
  -- active tracks (six strength + cardio). character_level matches the
  -- canonical `character_state` view + Dart `characterLevel(...)`. Denominator
  -- stays 4. See migration 00080 header for the never-regress invariant.
  SELECT
  ...
  -- lib/features/rpg/data/title_thresholds_table.dart row-for-row (UNCHANGED).
  ...
  -- Step 8.2: character-level crossings. Thresholds UNCHANGED (saga_eternal
  -- stays 148; the 172-cap title is Phase 38f). Only the v_*_char_level
  -- VALUES feeding this now include cardio.
  ...
  -- Step 8.3: cross-build distinction titles via 00043's helper (UNCHANGED).
  INSERT INTO public.earned_titles (user_id, title_id, earned_at, is_active)
...
-- ---------------------------------------------------------------------------
-- PART B — record_session_xp_batch (verbatim 00077 PART B + 7-part snapshots)
-- ---------------------------------------------------------------------------
...
BEGIN
  ...
  -- Step 8.1: body-part rank crossings (VALUES UNCHANGED from 00077).
  INSERT INTO public.earned_titles (user_id, title_id, earned_at, is_active)
  SELECT v_user_id, v.slug, v_now, FALSE
  FROM (VALUES
    ('arms_r5_vein_stirrer',       'arms',       5),
  ...
  -- Step 8.2: character-level crossings. Thresholds UNCHANGED (saga_eternal
  -- stays 148; the 172-cap title is Phase 38f).
  IF v_post_char_level > v_pre_char_level THEN
    INSERT INTO public.earned_titles (user_id, title_id, earned_at, is_active)
    SELECT v_user_id, v.slug, v_now, FALSE
    FROM (VALUES
  ...
  -- Step 8.3: cross-build distinction titles (UNCHANGED).
  INSERT INTO public.earned_titles (user_id, title_id, earned_at, is_active)
  SELECT v_user_id, cb.slug, v_now, FALSE
  FROM public.evaluate_cross_build_titles_for_user(v_user_id) cb
  ON CONFLICT (user_id, title_id) DO NOTHING;
