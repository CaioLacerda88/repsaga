-- 00084 — Phase Vitality-3: strength XP vitality gate (record_set_xp +
-- record_session_xp_batch) + the rpg_strength_vitality_mult helper.
-- ===========================================================================
-- THESIS. Gate strength set XP by per-body-part conditioning, mirroring the
-- cardio gate (migration 00081) but with three differences ONLY:
--   (a) FLOOR = 0.50 (cardio is 0.40) — the gentler comeback floor (D1).
--   (b) DENOMINATOR = vitality_ref_peak (the DECAYING reference peak,
--       migration 00083), NOT the frozen monotone vitality_peak. The decay is
--       what lets a detrained returner's multiplier recover (a frozen all-time
--       peak never could) — the science-consistency linchpin.
--   (c) PER BODY PART. Cardio is a single scalar; a strength set attributes
--       across multiple body parts, so vmult is a MAP keyed by attribution
--       body part: each bp's share is scaled by ITS OWN charge.
--
--   vpct  = clamp(vitality_ewma / vitality_ref_peak, 0, 1)  (ref_peak ≤ 0 → 1.0)
--   vmult = 0.50 + (1 - 0.50) × vpct
--   set_xp_for_bp = (existing 11-multiplier chain) × vmult   ← 12th/final factor
--
-- TIMING. vmult is computed from PRE-session vitality (read from
-- body_part_progress as it stands at the start of the save) — NON-CIRCULAR: the
-- save-time vitality recompute (recompute_vitality_for_user, 00082/00083) runs
-- AFTER these XP writes, exactly like the cardio gate. The EWMA is fed by VOLUME
-- LOAD, never by gated XP, so the gate can never feed its own input.
--
-- THROTTLE-ONLY (D6). Rank is never touched (saga inviolate) — only the XP
-- earn-rate scales. A fresh user (no body_part_progress row, or ref_peak 0) has
-- vmult 1.0 → full XP, so the gate is a no-op on the first load of a body part.
--
-- FORWARD-ONLY. No DROP, no down-migration. Signatures are unchanged
-- (CREATE OR REPLACE), so existing GRANTs are preserved and no schema changes.
-- The two RPC bodies are VERBATIM from 00081 except the gate lines, which are
-- flagged inline with "Phase Vitality-3".
-- ===========================================================================

-- ---------------------------------------------------------------------------
-- PART A — rpg_strength_vitality_mult helper
-- ---------------------------------------------------------------------------
-- vmult = 0.50 + 0.50 × clamp(ewma / ref_peak, 0, 1); ref_peak ≤ 0 → 1.0.
-- Mirrors the inline cardio math (00081) with FLOOR 0.50 and the ref_peak
-- denominator. IMMUTABLE: pure function of its two numeric args.
CREATE OR REPLACE FUNCTION public.rpg_strength_vitality_mult(
  p_ewma numeric,
  p_ref_peak numeric
)
RETURNS numeric
...
AS $$
  SELECT CASE
    WHEN COALESCE(p_ref_peak, 0) <= 0 THEN 1.0
    ELSE 0.50 + 0.50 * GREATEST(0.0, LEAST(1.0,
            COALESCE(p_ewma, 0) / p_ref_peak))
  END;
$$;

COMMENT ON FUNCTION public.rpg_strength_vitality_mult(numeric, numeric) IS
  'Phase Vitality-3 strength conditioning XP-gate: 0.50 + 0.50 × '
  'clamp(ewma/ref_peak,0,1); ref_peak ≤ 0 → 1.0 (un-conditioned no-op). '
  'Denominator is the DECAYING vitality_ref_peak (00083), keyed per body part.';

-- ---------------------------------------------------------------------------
-- PART B — record_set_xp (VERBATIM 00081 + Phase Vitality-3 per-bp gate)
-- ---------------------------------------------------------------------------
...
DECLARE
  ...
  -- Phase Vitality-3 — per-bp strength conditioning gate. v_vmult is read PER
  -- attribution body part from PRE-session vitality; v_dom_vmult records the
  -- dominant-part value for the audit payload.
  v_vmult           numeric;
  v_dom_vmult       numeric := 1.0;
BEGIN
  v_target_reps := NULL;
  ...
  LOOP
    ...
    -- Phase Vitality-3 — per-bp strength conditioning gate. Read THIS body
    -- part's PRE-session vitality (vitality_ewma + the DECAYING
    -- vitality_ref_peak, 00083) as it stands at the start of the save. The
    -- vitality columns are not mutated by this RPC (the save-time recompute
    -- runs AFTER), so this is non-circular. A never-loaded bp (no row, or
    -- ref_peak 0) → vmult 1.0 (the gate is a no-op on a bp's first load).
    SELECT public.rpg_strength_vitality_mult(
             COALESCE(bpp.vitality_ewma, 0), COALESCE(bpp.vitality_ref_peak, 0))
    INTO v_vmult
    FROM public.body_part_progress bpp
    WHERE bpp.user_id = v_user_id AND bpp.body_part = v_attr_key;
    IF v_vmult IS NULL THEN v_vmult := 1.0; END IF;
    ...
    v_xp_for_bp := v_base
                 ...
                 * v_frequency_mult
                 * v_attr_share
                 * v_vmult;   -- Phase Vitality-3 — 12th/final factor (per-bp)
...
-- ---------------------------------------------------------------------------
-- PART C — record_session_xp_batch (VERBATIM 00081 + Phase Vitality-3 gate)
-- ---------------------------------------------------------------------------
...
DECLARE
  ...
  v_pre_char_level  int;
  v_post_char_level int;
  -- Phase Vitality-3 — per-bp strength conditioning gate. v_vmult_by_bp is the
  -- PRE-session vmult for each of the 6 strength tracks (index 1..6, same
  -- ordering as v_weekly_vol; index 7 = cardio, never gated here). Read ONCE
  -- up-front (mirrors the weekly_vol pre-read) — non-circular, since the
  -- vitality columns aren't mutated until the save-time recompute runs after.
  v_vmult_by_bp      float8[] := ARRAY[1,1,1,1,1,1,1]::float8[];
  v_dom_vmult_f      float8;
  v_vit_chest        float8;
  v_vit_back         float8;
  v_vit_legs         float8;
  v_vit_shoulders    float8;
  v_vit_arms         float8;
  v_vit_core         float8;
BEGIN
  ...
  -- Phase Vitality-3 — per-bp PRE-session strength conditioning multiplier for
  -- the six strength tracks. Read ONCE here (same ordering as v_weekly_vol):
  -- vmult = rpg_strength_vitality_mult(vitality_ewma, vitality_ref_peak). A
  -- never-loaded bp (no row, or ref_peak 0) → 1.0 (gate no-op). Index 7 (cardio)
  -- stays 1.0 — cardio sets are excluded from this strength batch.
  SELECT
    COALESCE(MAX(public.rpg_strength_vitality_mult(
      COALESCE(bpp.vitality_ewma, 0), COALESCE(bpp.vitality_ref_peak, 0)))
        FILTER (WHERE bpp.body_part = 'chest'),     1.0)::float8,
    COALESCE(MAX(public.rpg_strength_vitality_mult(
      COALESCE(bpp.vitality_ewma, 0), COALESCE(bpp.vitality_ref_peak, 0)))
        FILTER (WHERE bpp.body_part = 'back'),      1.0)::float8,
    COALESCE(MAX(public.rpg_strength_vitality_mult(
      COALESCE(bpp.vitality_ewma, 0), COALESCE(bpp.vitality_ref_peak, 0)))
        FILTER (WHERE bpp.body_part = 'legs'),      1.0)::float8,
    COALESCE(MAX(public.rpg_strength_vitality_mult(
      COALESCE(bpp.vitality_ewma, 0), COALESCE(bpp.vitality_ref_peak, 0)))
        FILTER (WHERE bpp.body_part = 'shoulders'), 1.0)::float8,
    COALESCE(MAX(public.rpg_strength_vitality_mult(
      COALESCE(bpp.vitality_ewma, 0), COALESCE(bpp.vitality_ref_peak, 0)))
        FILTER (WHERE bpp.body_part = 'arms'),      1.0)::float8,
    COALESCE(MAX(public.rpg_strength_vitality_mult(
      COALESCE(bpp.vitality_ewma, 0), COALESCE(bpp.vitality_ref_peak, 0)))
        FILTER (WHERE bpp.body_part = 'core'),      1.0)::float8
  INTO v_vit_chest, v_vit_back, v_vit_legs,
       v_vit_shoulders, v_vit_arms, v_vit_core
  FROM public.body_part_progress bpp
  WHERE bpp.user_id = v_user_id;
  v_vmult_by_bp[1] := v_vit_chest;
  v_vmult_by_bp[2] := v_vit_back;
  v_vmult_by_bp[3] := v_vit_legs;
  v_vmult_by_bp[4] := v_vit_shoulders;
  v_vmult_by_bp[5] := v_vit_arms;
  v_vmult_by_bp[6] := v_vit_core;
  ...
  LOOP
    ...
    LOOP
      ...
      IF v_attr_key = v_dom_part THEN
        ...
        v_dom_cap_f     := v_cap_f;
        v_dom_vmult_f   := v_vmult_by_bp[v_bp_idx];
      END IF;
      ...
      v_xp_for_bp_f := v_base_f
                     ...
                     * v_frequency_mult_f
                     * v_attr_share
                     * v_vmult_by_bp[v_bp_idx];  -- Phase Vitality-3 — per-bp gate
    ...
    v_event_payload := jsonb_build_object(
      ...
      'vitality_mult',       round(v_dom_vmult_f::numeric, 4),
